...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  login: async (email, password, remember) => {
    set({ isLoading: true });
    try {
      const resp: any = await api<any>('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email, password }),
      });
      // Support both { token, user } and Supabase-like { access_token, token_type, refresh_token, expires_at, user }
      const token: string = resp?.token ?? resp?.access_token;
      const tokenType: string = resp?.token_type ?? 'bearer';
      const refreshToken: string | undefined = resp?.refresh_token;
      const expiresAt: number | undefined = resp?.expires_at;
      ...
      try {
        ...
        if (refreshToken) persist.setItem('auth_refresh_token', refreshToken);
        if (expiresAt) persist.setItem('auth_expires_at', String(expiresAt));
