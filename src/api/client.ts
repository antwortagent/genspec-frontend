...
export const API_BASE = (import.meta as any).env?.VITE_API_BASE ?? 'https://genspec-backend-core-production.up.railway.app';
...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (token) {
    (headers as Record<string, string>)['Authorization'] = `${tokenType} ${token}`;
  }
  ...
  if (!res.ok) {
    ...
    const err: any = new Error(`API ${res.status}: ${errorText || res.statusText}`);
...
// Example slices for later wiring
export type UIState = {
  busy: boolean;
